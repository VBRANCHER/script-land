class teste6 extends Phaser.Scene {
	...
	create ()
	{
		...
		this.startBtn.on('pointerdown', function (event) { testeObj6.updateVidas(); score = score - 5; 
		if (score < 0){
			score = 0;
		}
		carregarMoeda();});
		...
		this.startBtn.on('pointerdown', function (event) {game.scene.switch('teste6','teste7'); score = score + 10; carregarMoeda();});;
    ...
    {	
		...
		if (vidasRestantes < 0){
			vidasRestantes = 3;
			testeObj6.vidasText.setText('Vidas: ' + vidasRestantes);
			game.scene.switch('teste6','game_over2');
		}
    }
